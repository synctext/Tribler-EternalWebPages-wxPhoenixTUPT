...
class WebBrowser(XRCPanel):#IGNORE:R0902 gui class.
    ...
    def goBackward(self, event):#IGNORE:W0613 standard given argument.
        """Go to the previous page in history."""
        if self.webview.CanGoBack():
            self.webview.GoBack()
        
    def goForward(self, event):#IGNORE:W0613 standard given argument.
        """Go to the next page in history."""
        if self.webview.CanGoForward():
            self.webview.GoForward()
    
    def loadURLFromAdressBar(self, event):#IGNORE:W0613 standard given argument.
    ...
    def __UrlToPageSrc(self, url):
        """Get page source.
        Args:
            url (str) : url corresponding to the source needed
        Returns page source (str)."""
        try:
    ...
    def __notifyLoadedListeners(self, event):
        """Notify any listeners of this object.
        Args:
            event : The event that triggered the notification."""
        for listener in self.loadlisteners:
            try:
                #HTML source is retrieved because the html is not yet available from the webview.
                #Retrieving it would result in a segfault.
    ...
    def onURLNavigating(self, event):#IGNORE:W0613 always given event.
    ...
    def UserChangeText(self, event):#IGNORE:W0613 always given event.
        '''Callback for when a user changed the text in the url-bar'''
        self.__evtUserChangedText.set()
    
    def onURLLoaded(self, event):#IGNORE:W0613 always given event.
    ...
    def OnInfoBarMouseOver(self, event):#IGNORE:W0613 always given event.
    ...
    def OnInfoBarMouseOut(self, event):#IGNORE:W0613 always given event.
    ...
    def SetInfoBarContents(self, *orderedContents):
        """Set the contents of the infobar."""
    ...
    class NavigatingNewPageEvent(object):
        """Event used when navigating to a new webpage."""
        
        def __init__(self, url):
            self.url = url
            
        def GetURL(self):
            """Get the url of the new webpage (str)."""
            return self.url
    ...
    class WebpageLoadingGraphic(Thread):#IGNORE:R0902 gui class.
        ...
        def Paint(self, event):#IGNORE:W0613 always given event.
