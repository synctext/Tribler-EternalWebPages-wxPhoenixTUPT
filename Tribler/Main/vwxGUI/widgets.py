...
class MinMaxSlider(wx.Panel):
    ...
    def GetCurrentValues(self):
        length = self.range[1] - self.range[0]
        min_val = math.pow(max(self.arrow_up_rect[0] - self.range[0],0), self.base) / math.pow(max(length,0), self.base)
        max_val = math.pow(max(self.arrow_down_rect[0] - self.range[0],0), self.base) / math.pow(max(length,0), self.base)
